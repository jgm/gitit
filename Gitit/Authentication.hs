...
resetPasswordRequestForm _ params = do
  ...
  let contents = if null (mailCommand cfg)
                    then p << "Sorry, password reset not available.  No SMTP host found." 
                    else passwordForm
  formattedPage (defaultPageLayout
     { pgShowPageTools = False, pgTabs = [], pgTitle = "Reset your password" })
...
resetPasswordRequest _ params = do
  ...
  let errors = case mbUser of
        Nothing -> ["Unknown user. Please re-register or press back to try again."]
        Just u  -> if null . uEmail $ u
                      then ["Since you did not register with an email address, we can't reset your password."]
                      else []
  if null errors
    then do
      let response =
            p << [ stringToHtml "An email has been sent to ",
                 ...
                 br , stringToHtml "Please click on the enclosed link to reset your password."]
      liftIO $ sendReregisterEmail (fromJust mbUser)
      formattedPage (defaultPageLayout{ pgShowPageTools = False, pgTabs = [],
                      pgTitle = "Resetting your password" }) "_resetPassword"  params response
    else registerForm >>=
         formattedPage (defaultPageLayout { pgShowPageTools = False, pgTabs = [],
                      pgTitle = "Register for an account" }) "_register" (params { pMessages = errors })
...
sendReregisterEmail user = do
  ...
  let filledTemplate = T.render .
                       ...
                       messageTemplate
  let (mailcommand:args) = words $ substitute "%s" (uEmail user) (mailCommand cfg)
  (exitCode, _pOut, pErr) <- readProcessWithExitCode mailcommand args filledTemplate
  logM "gitit" WARNING $ "Sent reset password email to " ++ uUsername user ++ " at " ++
    uEmail user
...
resetPassword _ params = do
  ...
  let resetCodeMatches = take 20 (pHashed (uPassword (fromJust user))) == (pResetCode params)
  ...
  if null errors
     then resetPasswordForm user >>=
          formattedPage (defaultPageLayout { pgShowPageTools = False, pgTabs = [],
                  pgTitle = "Reset your registration info" }) "_doResetPassword" params
     else registerForm >>=
          formattedPage (defaultPageLayout { pgShowPageTools = False, pgTabs = [],
                   pgTitle = "Register for an account" }) "_register" (params { pMessages = [errors] })
  ...
  case result of
    Left errors ->
      resetPasswordForm mbUser >>=
          formattedPage (defaultPageLayout { pgShowPageTools = False, pgTabs = [],
            pgTitle = "Reset your registration info" }) "_register" (params { pMessages = errors })
    Right (uname', email, pword) -> do
       user <- liftIO $ mkUser uname' email pword
       adjustUser uname' user
       liftIO $ logM "gitit" WARNING $ "Successfully reset password and email for " ++ uUsername user
       loginUser "/" (params { pUsername = uname', pPassword = pword, pEmail = email })
...
sharedForm mbUser = do
  ...
  let userNameField = case mbUser of
                      Nothing    ->  label << "Username (at least 3 letters or digits):" +++ br +++
                                    textfield "username" ! [size "20"] +++ br
                      Just user  ->  label << ("Username (cannot be changed): " ++ (uUsername user)) +++ br
...
sharedValidation :: MonadIO m => ValidationType -> Params -> m (Either [String] (String,String,String))
sharedValidation validationType params = do
  ...
  taken <- isUser uname
  cfg <- getConfig
  let optionalTests Register = [(taken, "Sorry, that username is already taken.")]
...
loginForm = do
  cfg <- getConfig
  return $ gui "/_login" ! [identifier "loginForm"] << fieldset <<
     ...
     if null (mailCommand cfg)
        then noHtml
                  ...
                  , anchor ! [href "/_resetPassword"] << "click here to get a new one."]
...
loginUserForm' :: String -> Params -> Web Response
loginUserForm' page params =
  loginForm >>= formattedPage (defaultPageLayout { pgShowPageTools = False, pgTabs = [], pgTitle = "Login" }) page params
